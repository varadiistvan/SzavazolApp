    ...
    constructor(props) {
        ...
        for(var i = 0; i < 10; i++) {
            ...
            for(var j = 0; j < 4; j++) {
                ...
                kérdések[i].válaszok.push({key: j, szöveg: `Vízszintes ${x}, függőleges ${y}`, érték: {x: x, y: y}})
            }
        }
        ...
        if(!this.state.finished) {
                        ...
                        {this.state.kérdések[this.state.currentQuestion].válaszok.map(item => {
                            return(
                                <View key={item.key} style={{width: '100%', alignItems: 'center'}}> 
                                    <Pressable style={{borderColor: 'gray', borderWidth: 1, borderRadius: 20, width: '80%', backgroundColor: this.state.selected == item.key? 'green': 'transparent'}} onPress={() => {
                    ...
                    <View style={{flex: 2, flexDirection: 'row', justifyContent: 'space-around', width: '100%'}}>
                        ...
                        <Pressable style={{display: 'flex', flexDirection: 'row', alignItems: 'center', opacity: this.state.selected == null? 0.5: 1}} onPress={() => {
                            if(this.state.selected != null) {
                                ...
                                this.setState({coordinates: {x: this.state.coordinates.x + értékek.x > 10? 10: this.state.coordinates.x + értékek.x, y: this.state.coordinates.y + értékek.y > 10? 10: this.state.coordinates.y + értékek.y}, currentQuestion: this.state.currentQuestion + 1, selected: null})
                            }
            ...
            return(
                <SafeAreaView style={{justifyContent: 'space-evenly', alignItems: 'center', flex: 1, height: '100%'}}>
                    <View style={{width: '80%', aspectRatio: 1, flex: 1}}>
                        <Svg width="100%" height='100%' viewBox="0 0 220 220" >
                            ...
                            <SvgText fill="red" strokeWidth="0.3" stroke="red" fontSize="8" fontFamily="Poppins_400Regular" x={this.state.coordinates.x*10+110} y={this.state.coordinates.y*-10 + 100} textAnchor="middle" >
                                A Te eredményed
                            </SvgText>
